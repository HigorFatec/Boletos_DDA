...
def formboleto(banco, n_documento, data_emissao_formatada, valor_formatado, data_vencimento_formatada, agencia, Codigo_de_Barras, carteira,telefone_empresa,sacado,endereco,endereco1,valor,vencimento,cedente,especiedoc,aceite,especiemon,nossonumero,valorexpresso,juros,observacao1,observacao2,observacao3,codigobarra,localpagamento,usobanco):
  ...
  #Pegando dados para criar as pastas
  dia, mes, ano = vencimento.split('/')


  caminho_diretorio = f'BOLETO/{ano}/{mes}/{dia}/'

  # Verificar se o diretório existe
  if not os.path.exists(caminho_diretorio):
      # Se não existir, criar os diretórios necessários
      os.makedirs(caminho_diretorio)

  documento_sem_espacos = documento.replace(" ", "")
  documento_sem_espacos = documento_sem_espacos.replace("/", "_")
  cedente = cedente.replace("/", "_")


  boleto=Canvas(f'BOLETO/{ano}/{mes}/{dia}/boleto_{cedente}_{documento_sem_espacos}.pdf')
