...
def processar_linha(nome_arquivo, linha, inscricao):
    ...
    data_emissao_formatada = data_obj.strftime("%Y-%m-%d")

    data_obj2 = datetime.strptime(vencimento_titulo, "%d%m%Y")
    data_vencimento_formatada = data_obj2.strftime("%Y-%m-%d")
    ...
    try:
        ...
        IF NOT EXISTS (SELECT 1 FROM BASEDDA WHERE LINDIG = '{Codigo_de_Barras}')

        BEGIN

        INSERT INTO BASEDDA (EMPCGC,FORCGC,RAZSOC,DATEMI,DATVEN,VLRTIT,NUMDOC,LINDIG,DATINC) VALUES ('{cnpj_gcp}','{cnpj_fornecedor}','{nome}','{data_emissao_formatada}','{data_vencimento_formatada}','{valor_formatado}','{n_documento}','{Codigo_de_Barras}',GETDATE())
